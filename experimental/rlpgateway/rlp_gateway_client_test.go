...
import (
	"bytes"
	"code.cloudfoundry.org/go-loggregator/experimental/rlpgateway"
	"errors"
	"fmt"
	"io"
	"io/ioutil"
...
var _ = Describe("RlpGatewayClient", func() {
	...
	BeforeEach(func() {
		...
		c = rlpgateway.NewRLPGatewayClient(
			"https://some.addr",
			rlpgateway.WithRLPGatewayHTTPClient(spyDoer),
			rlpgateway.WithRLPGatewayClientLogger(log.New(logBuffer, "", 0)),
		)
	})
